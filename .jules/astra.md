...
## 2025-08-10 - [Failure Resilience: Expose Status Code to Retry Wrappers]
**Learning:** The `executeWithRetry` wrapper uses `AIError.fromError` which calls `classifyError(error)`. `classifyError` expects `error.status` to be set to accurately classify errors like 429 Too Many Requests or 500 Internal Server Error as retryable. If the underlying `fetch` call throws a generic `Error` without attaching `response.status`, `classifyError` cannot correctly identify these rate-limiting or server issues, breaking the exponential backoff mechanism.
**Action:** Whenever a `fetch` wrapper encounters `!response.ok`, it must throw an `Error` and explicitly attach `response.status` to it (e.g., `(error as any).status = response.status;`) before throwing, allowing outer exponential backoff wrappers to correctly intercept and handle transient network/API failures.
