...
## 2026-08-28 - [Failure Resilience: Propagating HTTP Status to executeWithRetry wrappers]
**Learning:** The `executeWithRetry` utility expects `Error` objects with an embedded `.status` property to correctly classify them as rate limits (429) or server errors (500) that should be retried using exponential backoff. If API wrappers throw generic `Error` instances without this `status` property on `!response.ok`, `executeWithRetry` treats them as non-retryable fatal errors, bypassing the retry logic.
**Action:** When validating `!response.ok` inside custom fetch wrappers, always attach the HTTP response status to the thrown error using `error.status = response.status` to ensure downstream retry managers can correctly classify and recover from transient failures.
