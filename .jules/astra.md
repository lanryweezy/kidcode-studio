...
## 2025-06-25 - [Missing AI Action Handler & Contract Enforcement]
**Learning:** If a frontend service expects to call a specific AI action (e.g. `testGame` in `/api/gemini`) but the backend proxy lacks a handler for that action, the request will fail with a 400 error and silently fall back to local handling. When adding the missing handler, failing to strictly enforce the output JSON schema (as expected by the frontend) can lead to mismatched parsing and silent crashes.
**Action:** When implementing new AI action handlers, always explicitly define the JSON schema in the prompt to match the frontend interface expectations (e.g., `{ title, description, priority }`), sanitize the markdown response, and use a try/catch block around `JSON.parse` to ensure gracefully degradation to empty arrays on parsing failures.
