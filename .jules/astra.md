...
## 2025-07-28 - [Failure Resilience: Propagating HTTP Status to Retry Wrappers]
**Learning:** When AI API calls return non-OK status codes (like 429 Too Many Requests or 5xx Server Errors), throwing a generic Error without the HTTP status attached prevents higher-level exponential backoff wrappers (like `executeWithRetry`) from classifying the error correctly. This results in either missing retries for transient errors or improperly retrying client-side 4xx errors.
**Action:** Always validate `!response.ok` within fetch calls inside retry wrappers, and explicitly attach the HTTP `.status` property to the thrown `Error` object so that the retry mechanism can handle rate limits and transient server errors gracefully.
