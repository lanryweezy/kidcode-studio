...
import { Triangle, Heart, ArrowLeft, ToggleLeft, ToggleRight, Search, Menu, Send } from 'lucide-react';
...
const GameStage = React.memo((props: any) => {
    const { isExecuting, onInput } = props;
...
const HardwareStage = React.memo(({ hardwareState, hardwareStateRef, circuitComponents, pcbColor, onCircuitUpdate, isExecuting, onHardwareInput }: any) => {
    ...
    // Fast Visual Update Loop (60FPS) - ALWAYS RUNS
    useEffect(() => {
        let frameId: number;
    ...
    // CALCULATE PIN COORDINATES TO MATCH SVG LAYOUT
    const getPinCoords = (pin: number) => {
        const isLeft = pin % 2 === 0; 
        const row = Math.floor(pin / 2); 
        
        // Match the SVG translate(150, 120) logic
        const x = isLeft ? 90 : 210;
        const y = 130 + (row * 20);
        
        // Fallback for high pin numbers (sensors, breadboard connections)
    ...
    return (
        <div 
            className="relative w-[300px] h-[400px] border-4 border-slate-700 bg-slate-800 rounded-xl shadow-2xl overflow-hidden select-none"
            ref={svgRef as any}
            ...
            onDrop={(e) => {
                ...
                try {
                    ...
                    if (tool && tool.type) {
                       onCircuitUpdate([...circuitComponents, { id: crypto.randomUUID(), type: tool.type, x: 150, y: 280, pin: tool.defaultPin || 0, rotation: 0 }]);
                       playSoundEffect('click');
                    }
                } catch(e) {}
            }}
        ...
        >
            <svg width="100%" height="100%" viewBox="0 0 300 400" className="w-full h-full pointer-events-auto">
                ...
                {localComponents.map((comp: any) => {
                    ...
                    return (
                        <path 
                            key={`wire-${comp.id}`} 
                            id={`wire-${comp.id}`}
                            ref={(el) => { if(el) wireRefs.current.set(`wire-${comp.id}`, el); else wireRefs.current.delete(`wire-${comp.id}`); }}
                            d={`M ${startX} ${startY} C ${startX} ${(startY + pinCoords.y)/2}, ${pinCoords.x} ${(startY + pinCoords.y)/2}, ${pinCoords.x} ${pinCoords.y}`} 
                            fill="none" 
                            stroke={wireColor} 
                ...
                {/* Microcontroller Representation with Labels */}
                <g transform="translate(150, 120)">
                    ...
                    {/* Pins Left: x = -60, translate(0, i*20) -> relative x=0 */}
                    <g transform="translate(-60, 10)">
                        {[0, 2, 4, 6].map((p, i) => (
                            <g key={p} transform={`translate(0, ${i * 20})`}>
                                <text x="-8" y="3" fontSize="6" fill="#cbd5e1" textAnchor="end" fontWeight="bold">D{p}</text>
                                <circle cx="0" cy="0" r="3" fill="#fbbf24" stroke="#d97706" strokeWidth="1" />
                            </g>
                        ))}
                    </g>
                    {/* Pins Right: x = 60, translate(0, i*20) -> relative x=0 */}
                    <g transform="translate(60, 10)">
                        {[1, 3, 5, 7].map((p, i) => (
                            <g key={p} transform={`translate(0, ${i * 20})`}>
                                <text x="8" y="3" fontSize="6" fill="#cbd5e1" textAnchor="start" fontWeight="bold">D{p}</text>
                                <circle cx="0" cy="0" r="3" fill="#fbbf24" stroke="#d97706" strokeWidth="1" />
                            </g>
                        ))}
                    </g>
                ...
                {localComponents.map((comp: any) => (
                    <g key={comp.id} 
                       transform={`translate(${comp.x}, ${comp.y}) rotate(${comp.rotation || 0} 10 10)`} 
                       ref={(el) => { if(el) componentRefs.current.set(comp.id, el); else componentRefs.current.delete(comp.id); }}
                    ...
                    >
                        ...
                        {/* Dynamic Part (LED, Arm, etc) identified by class 'light-part' or 'servo-arm' */}
                        ...
                        {comp.type === 'BUTTON' && (
                            <circle 
                                cx="10" cy="10" r="6" fill="#ef4444" stroke="#991b1b" strokeWidth="2"
                                className="cursor-pointer"
                                onPointerDown={(e) => {
                                    e.stopPropagation(); // Stop drag start
                                    onHardwareInput?.(comp.pin, true);
                                }}
                                onPointerUp={(e) => {
                                    e.stopPropagation();
                                    onHardwareInput?.(comp.pin, false);
                                }}
                                onPointerLeave={(e) => {
                                    onHardwareInput?.(comp.pin, false);
                                }}
                        ...
                        {comp.type === 'LCD' && (
                            <>
                                <rect x="-5" y="0" width="30" height="20" fill="#22c55e" opacity="0.8" rx="2" />
                                <text x="10" y="12" fontSize="4" textAnchor="middle" fontFamily="monospace">LCD</text>
                            </>
                        )}
                        {comp.type === 'OLED' && (
                            <>
                                <rect x="-5" y="0" width="30" height="20" fill="#000" stroke="#333" rx="2" />
