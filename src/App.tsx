...
export const App: React.FC = () => {
  ...
  const [showAI3DCreator, setShowAI3DCreator] = useState(false);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 'b') {
        e.preventDefault();
        setLeftPanelWidth(prev => prev === 0 ? 300 : 0);
      }
    };
    ...
    <div className={`h-screen flex flex-col overflow-hidden ${darkMode ? 'dark bg-slate-950 text-white' : 'bg-slate-50 text-slate-800'} ${hackerMode ? 'hacker-mode' : ''}`}>
        ...
        <Suspense fallback={<div className="h-screen flex items-center justify-center bg-slate-900 text-white font-black animate-pulse">LOADING KIDCODE STUDIO...</div>}>
            ...
            <Suspense fallback={null}>
                ...
                {showPixelEditor && <PixelEditor onClose={() => setShowPixelEditor(false)} onSave={(img) => { updateSpriteState({ texture: img, emoji: '' }); setShowPixelEditor(false); }} currentImage={spriteState.texture} />}
                ...
                {showVariables && <VariableMonitor variables={mode === AppMode.APP ? appState.variables : spriteState.variables} onClose={() => setShowVariables(false)} />}
                {showMissions && <MissionOverlay mission={activeMission} onClose={() => setShowMissions(false)} />}
                {showCodePageManager && (
                ...
                {showTutorial && <TutorialSystem onClose={() => setShowTutorial(false)} />}
            </Suspense>
            <NPCModal />
