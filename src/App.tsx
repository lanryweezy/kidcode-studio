...
export const App: React.FC = () => {
  ...
  useEffect(() => {
      gameCanvasSizeRef.current = gameCanvasSize;
  }, [gameCanvasSize]);
  ...
  const handleUpgrade = (plan: PlanType) => {
      ...
      // Simulate cloud sync upgrade effect
  ...
  const handleGenerateSprite = async () => {
      const prompt = window.prompt("What should your sprite look like? (e.g. 'a cute blue dragon')");
      if (!prompt) return;
      ...
      if (imgData) {
          ...
          alert("Could not generate sprite. Try again!");
      }
  ...
  useEffect(() => {
      ...
      const physicsInterval = setInterval(() => {
          ...
          // Dynamic Bounds based on resized canvas
          const canvasW = gameCanvasSizeRef.current.w;
          const canvasH = gameCanvasSizeRef.current.h;
          const GROUND_Y = canvasH - 60; // 60px padding for floor
