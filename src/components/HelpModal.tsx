...
const HelpModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="w-full max-w-4xl bg-white rounded-[2.5rem] shadow-2xl border-4 border-violet-500 overflow-hidden flex flex-col h-[600px] animate-in zoom-in-95 duration-300">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-64 bg-slate-50 border-r border-slate-100 p-4 space-y-2">
            <button 
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold transition-all ${activeTopic === 'app' ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/30 translate-x-2' : 'hover:bg-slate-100:bg-slate-800 text-slate-500'}`}
            >
              <Layout size={20} /> App Builder
            </button>
            <button 
              onClick={() => setActiveTab('game')}
              className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold transition-all ${activeTopic === 'game' ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/30 translate-x-2' : 'hover:bg-slate-100:bg-slate-800 text-slate-500'}`}
            >
              <Gamepad2 size={20} /> Game Maker
            </button>
            <button 
              onClick={() => setActiveTab('hw')}
              className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold transition-all ${activeTopic === 'hw' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30 translate-x-2' : 'hover:bg-slate-100:bg-slate-800 text-slate-500'}`}
            >
              ...
              <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                <section>
                  ...
                  <p className="text-slate-600 font-medium leading-relaxed mb-4">
                    Want to share data between screens? Use variables that start with <code className="bg-slate-100 px-2 py-1 rounded text-blue-500 font-bold">global_</code>!
                  </p>
                  <div className="bg-blue-50 p-4 rounded-2xl border-2 border-blue-100">
                    <p className="text-sm font-bold text-blue-600 mb-2">Example:</p>
            ...
            {activeTopic === 'game' && (
              <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                ...
                <section>
                  ...
                  <p className="text-slate-600 font-medium mb-4">
                    Tiles are now solid! You can stand on <span className="font-bold text-slate-800">Bricks</span> or jump on <span className="font-bold text-pink-500">Springs</span>. Watch out for <span className="font-bold text-red-500">Lava</span>!
                  </p>
                </section>
              </div>
            )}
