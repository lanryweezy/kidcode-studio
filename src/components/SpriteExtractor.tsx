...
const SpriteExtractor: React.FC<{ onSpriteExtracted: (frames: string[]) => void, onClose: () => void }> = ({ onSpriteExtracted, onClose }) => {
    ...
    const sliceFrames = () => {
        ...
        const cols = Math.floor(image.width / gridSize.w);
        const rows = Math.floor(image.height / gridSize.h);
        const newFrames: string[] = [];

        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const offCanvas = document.createElement('canvas');
                offCanvas.width = gridSize.w;
                offCanvas.height = gridSize.h;
                const offCtx = offCanvas.getContext('2d');
                if (offCtx) {
                    offCtx.drawImage(image, c * gridSize.w, r * gridSize.h, gridSize.w, gridSize.h, 0, 0, gridSize.w, gridSize.h);
                    newFrames.push(offCanvas.toDataURL());
                }
            }
        }
        setFrames(newFrames);
