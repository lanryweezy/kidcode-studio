...
export const AVAILABLE_BLOCKS: Record<AppMode, BlockDefinition[]> = {
    ...
    [AppMode.GAME]: [
        ...
        { type: CommandType.CREATE_3D_OBJECT, label: 'Create 3D Object', icon: Box, defaultParams: { text: 'cube', x: 0, y: 0, z: 0 }, color: 'bg-cyan-700', category: '3D', description: 'Create a 3D primitive shape.' },
        { type: CommandType.SET_3D_ROTATION, label: 'Rotate 3D Object', icon: RotateCw, defaultParams: { text: 'object1', x: 0, y: 0, z: 0 }, color: 'bg-cyan-500', category: '3D', description: 'Set rotation of a 3D object.' },
        { type: CommandType.SET_3D_MATERIAL, label: 'Set 3D Material', icon: PaintBucket, defaultParams: { text: 'object1', color: '#ff0000', value: 0.5 }, color: 'bg-pink-500', category: '3D', description: 'Change material color and properties.' },
        { type: CommandType.ADD_3D_LIGHT, label: 'Add 3D Light', icon: Lightbulb, defaultParams: { text: 'point', x: 0, y: 5, z: 0, color: '#ffffff', intensity: 1 }, color: 'bg-amber-500', category: '3D', description: 'Add a light to the 3D scene.' },
    ...
    [AppMode.CAD]: [
        { type: CommandType.CREATE_BOX, label: 'Create Box', icon: Box, defaultParams: { width: 30, height: 30, depth: 30 }, color: 'bg-cyan-600', category: 'Primitives', description: 'Create a box shape.' },
        { type: CommandType.CREATE_CYLINDER, label: 'Create Cylinder', icon: Box, defaultParams: { radius: 15, height: 30 }, color: 'bg-cyan-600', category: 'Primitives', description: 'Create a cylinder shape.' },
        { type: CommandType.CREATE_SPHERE, label: 'Create Sphere', icon: Box, defaultParams: { radius: 15 }, color: 'bg-cyan-600', category: 'Primitives', description: 'Create a sphere shape.' },
        { type: CommandType.EXTRUDE, label: 'Extrude', icon: ArrowUp, defaultParams: { distance: 20 }, color: 'bg-blue-500', category: 'Operations', description: 'Extrude a 2D shape into 3D.' },
        { type: CommandType.REVOLVE, label: 'Revolve', icon: RotateCcw, defaultParams: { angle: 360 }, color: 'bg-violet-500', category: 'Operations', description: 'Revolve a profile around an axis.' },
        { type: CommandType.SWEEP, label: 'Sweep', icon: Move, defaultParams: {}, color: 'bg-pink-500', category: 'Operations', description: 'Sweep a profile along a path.' },
        { type: CommandType.LOFT, label: 'Loft', icon: Layers, defaultParams: {}, color: 'bg-amber-500', category: 'Operations', description: 'Loft between two profiles.' },
        { type: CommandType.BOOLEAN_UNION, label: 'Union', icon: Target, defaultParams: {}, color: 'bg-green-500', category: 'Boolean', description: 'Merge two objects together.' },
        { type: CommandType.BOOLEAN_SUBTRACT, label: 'Subtract', icon: Eraser, defaultParams: {}, color: 'bg-red-500', category: 'Boolean', description: 'Subtract one object from another.' },
        { type: CommandType.BOOLEAN_INTERSECT, label: 'Intersect', icon: Scan, defaultParams: {}, color: 'bg-purple-500', category: 'Boolean', description: 'Keep only the intersection.' },
        { type: CommandType.EXPORT_STL, label: 'Export STL', icon: Download, defaultParams: { format: 'stl_binary' }, color: 'bg-emerald-500', category: 'Export', description: 'Export as STL for 3D printing.' },
        { type: CommandType.CAD_SET_PARAM, label: 'Set Parameter', icon: Settings, defaultParams: { varName: 'width', value: 30 }, color: 'bg-orange-500', category: 'Parameters', description: 'Change a parameter value.' },
    ],
};
