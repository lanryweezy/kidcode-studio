    ...
    try {
        switch (cmd.type) {
            ...
            case CommandType.SET_VIEW_3D:
                spriteStateRef.current.is3D = cmd.params.condition === 'true';
                break;
            ...
            case CommandType.GENERATE_ENVIRONMENT:
                if (spriteStateRef.current.is3D) {
                    // Logic for environment generation placeholder
                    setConsoleLogs(prev => [...prev, `🌍 Generating environment: ${cmd.params.text}...`].slice(-50));
                }
                break;
