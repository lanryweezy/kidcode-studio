...
export const generateCode = (
  ...
  code += `// Generated by KidCode Studio\n`;
  code += `// https://kidcode.studio\n\n`;
  ...
  if (mode === AppMode.HARDWARE) {
    ...
    code += `const sprite = new Sprite('\u{1F916}');\n\n`;
    code += `// Main game loop\n`;
  ...
  commands.forEach(cmd => {
    ...
    switch (cmd.type) {
      ...
      case CommandType.SAY_THINK:
        code += `${i}sprite.${cmd.params.message || 'say'}("${cmd.params.text}");\n`;
        break;
      case CommandType.SWITCH_COSTUME:
        code += `${i}sprite.setCostume("${cmd.params.text}");\n`;
        break;
      case CommandType.GHOST_EFFECT:
        code += `${i}sprite.setGhostEffect(${cmd.params.value || 50});\n`;
        break;
      case CommandType.COLOR_EFFECT:
        code += `${i}sprite.setColorEffect(${cmd.params.value || 0});\n`;
        break;
      case CommandType.SIZE_EFFECT:
        code += `${i}sprite.setSizeEffect(${cmd.params.value || 100});\n`;
        break;
      case CommandType.GLIDE_TO_XY:
        code += `${i}await sprite.glideTo(${cmd.params.x || 0}, ${cmd.params.y || 0}, ${cmd.params.value || 1});\n`;
        break;
      case CommandType.POINT_TOWARDS:
        code += `${i}sprite.pointTowards("${cmd.params.text || 'enemy'}");\n`;
        break;
      case CommandType.DISTANCE_TO:
        code += `${i}${cmd.params.varName} = sprite.distanceTo("${cmd.params.text || 'enemy'}");\n`;
        break;
      case CommandType.ASK_AND_WAIT:
        code += `${i}const answer = prompt("${cmd.params.text || 'Question?'}");\n`;
        break;
      case CommandType.ANSWER:
        code += `${i}${cmd.params.varName} = lastAnswer;\n`;
        break;
      case CommandType.WAIT_UNTIL:
        code += `${i}while (!(${cmd.params.condition || 'true'})) { await delay(16); }\n`;
        break;
      case CommandType.SET_GRAVITY_STRENGTH:
        code += `${i}sprite.gravityForce = ${cmd.params.value || 0.5};\n`;
        break;
      case CommandType.SET_CAMERA_TARGET:
        code += `${i}camera.follow("${cmd.params.text || 'player'}");\n`;
        break;
      case CommandType.DELETE_THIS_CLONE:
        code += `${i}return;\n`;
        break;
      case CommandType.PEN_DOWN:
        code += `${i}sprite.penDown();\n`;
        break;
      case CommandType.PEN_UP:
        code += `${i}sprite.penUp();\n`;
        break;
      case CommandType.PEN_CLEAR:
        code += `${i}sprite.clearPen();\n`;
        break;
      case CommandType.PEN_COLOR:
        code += `${i}sprite.setPenColor("${cmd.params.color || '#ff0000'}");\n`;
        break;
      ...
      case CommandType.MATH_OP:
        code += `${i}${cmd.params.varName} = Math.${cmd.params.text}(${cmd.params.value});\n`;
        break;
      case CommandType.RANDOM_BETWEEN:
        code += `${i}${cmd.params.varName} = Math.floor(Math.random() * (${cmd.params.value2} - ${cmd.params.value} + 1)) + ${cmd.params.value};\n`;
        break;
      ...
      case CommandType.STRING_CONTAINS:
        code += `${i}${cmd.params.varName} = "${cmd.params.text}".includes("${cmd.params.text2}");\n`;
        break;
