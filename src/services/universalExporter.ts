...
export type ExportFormat = 'html5' | 'windows' | 'mac' | 'linux' | 'android' | 'ios' | 'webgl' | 'zip';
...
export const exportToZip = async (
  ...
  try {
    ...
    // Add a simple README
    zip.file('README.md', `# ${options.projectName || 'KidCode Project'}\n\nGenerated by KidCode Studio.\nOpen \`index.html\` in any web browser to run your project.`);
...
export const exportProject = async (
  ...
  switch (options.format) {
    ...
    case 'webgl':
      ...
      case 'zip':
        return exportToZip(commands, mode, options);
    case 'windows':
      return exportToWindows(commands, mode, options);
    case 'mac':
...
export const getExportFormatInfo = (format: ExportFormat) => {
  const info: Record<ExportFormat, { label: string; icon: string; description: string; platforms: string[] }> = {
    ...
    zip: {
      label: 'ZIP Archive',
      icon: '📦',
      description: 'Download as a self-contained ZIP file',
...
export const EXPORT_PRESETS = [
  ...
  },
  {
    ...
    description: 'Self-contained offline HTML5 package',
    icon: '📦'
  },
  {
    id: 'windows_play',
